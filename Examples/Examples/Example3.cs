...
namespace Examples
{
    ...
    public sealed unsafe class Example3
    {
        public static void StartServer(ushort port, string localIP = "0.0.0.0", bool ipv6 = true)
        {
            ...
            Socket server = UdpPal.Create(ipv6);
            ...
            if (UdpPal.SetHostName(ref listenAddress, localIP) == SocketError.Success)
                Console.WriteLine($"SocketAddress set! {listenAddress}");

            error = UdpPal.Bind(server, ref listenAddress);
            if (error == 0)
            ...
            error = UdpPal.GetAddress(server, ref localAddress);
            Console.WriteLine($"Server local: {error} {localAddress}");

            SocketAddress address = new SocketAddress();
            byte* buffer = stackalloc byte[1024];

            error = UdpPal.GetHostName(ref localAddress, MemoryMarshal.CreateSpan(ref *buffer, 1024));
        ...
        public static void StartClient(string serverIP, ushort port, ushort localPort, bool ipv6 = false)
        {
            ...
            Socket client = UdpPal.Create(ipv6);
            ...
            if (UdpPal.SetIP(ref listenAddress, ipv6 ? "::0" : "0.0.0.0") == SocketError.Success)
            ...
            int a = UdpPal.Send(client, ref *buffer, bytes);
